import type { HHMMTime, LessonStructure } from './lesson';
...
export class Lesson {
  ...
  private timeCron(hhmmTime: string): string {
    ...
    return `${secondMinuteHour} * * ${this.lesson.weekOfDay}`;
  }
   ...
   */
  get StartTimeCron() {
    return this.timeCron(this.lesson.startAt);
  }
   ...
   */
  get EndTimeCron() {
    return this.timeCron(this.lesson.endAt);
  }
